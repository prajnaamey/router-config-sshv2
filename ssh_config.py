    ...
    while True:
        ...
        for ip in ip_list:
            ...
            if (len(a) == 4) and (1 <= int(a[0]) <= 223) and (int(a[0]) != 127) and (int(a[0]) != 169 or int(a[1]) != 254) and (0 <= int(a[1]) <= 255 and 0 <= int(a[2]) <= 255 and 0 <= int(a[3]) <= 255):
                ...
                print """There was an INVALID IP address!
    ...
    while True:
        for ip in ip_list:
            ...
            if ping_reply == 0:
                ...
                print "No response from device %s." % ip
                check2 = False
                break
        ...
        if check2 is False:
            ...
            print """All devices are reachable.
                     Waiting for username/password file."""
            break
...
def user_is_valid():
    ...
    while True:
        ...
        if os.path.isfile(user_file) is True:
            print """Username/password file has been validated.
                     Waiting for command file."""
            break
...
def cmd_is_valid():
    ...
    while True:
        ...
        if os.path.isfile(cmd_file) is True:
            print "Sending command(s) to device(s)."
            break

        else:
            print """File %s does not exist!
                     Please check and try again!""" % cmd_file
            continue
